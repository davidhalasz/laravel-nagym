        ...
        <div class="mb-6">
            ...
            <table class="table table-bordered p-6 w-full">
                @foreach ($eloterjesztesek as $eloterjesztes)
                    @if ($eloterjesztes->title_id == $cim->id)
                        <tr>
                            <td class="border-b border-slate-300 py-4 px-6">
                                <a class="text-blue-500 hover:text-blue-700" href="{{ url('/storage/eloterjesztesek/'.$eloterjesztes->filepath) }}">
                                    {{ $eloterjesztes->filename }}
                                </a> 
                            </td>
                            <td class="border-b border-slate-300 py-4 px-6 text-right">
                                <button wire:click="delete({{ $eloterjesztes->id }})"
